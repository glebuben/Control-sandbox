...
2. **LQR Phase:** Once the state enters a small neighborhood of the upright equilibrium ($|\theta| < \theta_{\mathrm{th}}$, $|\dot{\theta}| < {\dot{\theta}}_{\mathrm{th}}$, $|x|$, $|\dot{x}| < \dot{x}_{\mathrm{th}}$), the controller smoothly switches to a locally optimal Linear Quadratic Regulator (LQR) to achieve precise asymptotic stabilization and eliminate residual oscillations.
