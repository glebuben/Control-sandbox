...
In the nominal nonlinear mechanical model, the control input was defined as the motor torque:

$$
u=\tau_m.
$$

This assumption is convenient for Lyapunov-based backstepping design because the motor torque appears directly in the motor-side mechanical equation. However, in a real electromechanical actuator, torque is usually not commanded as an independent physical quantity. Instead, the motor drive controls the electric current, and the electromagnetic torque is generated by this current.

Therefore, a more physically interpretable actuator model can be introduced through the motor current.

---

### 13.1. Torque Generation by Motor Current

For a DC motor, or for an electric drive with a fast inner current-control loop, the electromagnetic torque is approximately proportional to the motor current:

$$
\tau_m=K_t i,
$$

where \(i\) is the motor current and \(K_t\) is the motor torque constant.
...
Thus, instead of assuming that the controller directly applies torque, we can assume that the controller commands motor current:

$$
u=i.
$$

The motor torque is then generated physically as

$$
\tau_m=K_tu.
$$

This gives a clearer electromechanical interpretation of the actuator.

---

### 13.2. Current-Controlled Flexible-Joint Model

Using the same mechanical variables,
...
$$\tau_c=k(\theta_m-\theta_l)+k_3(\theta_m-\theta_l)^3+b(\omega_m-\omega_l).$$

The load-side dynamics remain unchanged:

$$J_l\dot{\omega}_l=\tau_c-T_{f,l}(\omega_l).$$

The motor-side dynamics now include the current-generated torque:

$$J_m\dot{\omega}_m=K_t i-\tau_c-T_{f,m}(\omega_m).$$

Therefore, the current-controlled nonlinear state-space model is
...
$$\dot{x}_4=\frac{1}{J_m}\left[K_t i-\tau_c-T_{f,m}(x_4)\right].$$

Since the control input is now the current,
...
Thus, the system remains input-affine, but the input has a direct physical meaning as motor current.

---

### 13.3. Relation to the Torque-Based Controller

In the torque-controlled model, the backstepping controller computes the desired motor torque:

$$
\tau_m^*.
$$

For the current-controlled actuator, this desired torque is converted into the desired current:

$$i^*=\frac{\tau_m^*}{K_t}.$$

Therefore, the current command is

$$u=i^*=\frac{\tau_m^*}{K_t}.$$

If the motor drive tracks the commanded current sufficiently fast, then

$$
K_t i \approx \tau_m^*.
$$

In this case, the current-controlled implementation reproduces the same mechanical torque that was assumed in the theoretical torque-input model.

---

### 13.4. Backstepping Interpretation

The original backstepping design can be interpreted as a two-stage procedure.

First, the nonlinear mechanical controller computes the required torque:

$$
\tau_m^*.
$$

Second, the actuator converts this torque command into a current command:
...
This means that the current command is the real actuator input, while the motor torque is an intermediate physical effect generated by the motor.

---

### 13.5. Lyapunov Consistency

The torque-based backstepping proof assumes that the motor applies the desired torque exactly:
...
$$\tau_m=K_t\left(\frac{\tau_m^*}{K_t}\right)=\tau_m^*.$$

Therefore, the mechanical closed-loop error dynamics remain unchanged. The same Lyapunov proof applies because the motor-side torque generated by the current is exactly equal to the torque required by the backstepping controller.

If the original Lyapunov derivative was

$$\dot{V}=-c_1z_1^2-c_2z_2^2-c_3z_3^2,$$

then the same result is preserved under ideal current tracking:
...
Thus, the current-controlled implementation preserves the nominal asymptotic stability result of the torque-based design.

---

### 13.6. Practical Meaning

The current-controlled model is more realistic than direct torque control because current is a measurable and controllable electrical quantity. At the same time, it is still simpler than a full voltage-controlled electromechanical model because the electrical current dynamics are not explicitly included.

This assumption is realistic when the motor drive has a fast inner current loop. In that case, the outer nonlinear controller computes the desired current, while the motor electronics ensure that the actual current follows the command.

The main practical limitation is current saturation:

$$
|i|\le i_{\max}.
$$

Since motor torque is proportional to current,

$$
|\tau_m|\le K_t i_{\max}.
$$

Therefore, actuator saturation limits the maximum torque that can be generated by the controller. If the required current exceeds this limit, exact Lyapunov cancellation may no longer hold, and the stability result becomes local or semi-global depending on the available current authority.

---

### 13.7. Final Interpretation

The current-controlled formulation provides a practical bridge between the theoretical nonlinear controller and a real electromechanical actuator.

The torque-input model is useful for deriving the controller:

$$
u=\tau_m.
$$

The current-input model is useful for implementation:
...
Thus, the final controller can be written as

$$
i^*=\frac{\tau_m^*}{K_t}.
$$

This means that the backstepping controller computes the torque required to stabilize the nonlinear flexible-joint system, and this torque is physically realized by commanding the corresponding motor current.
